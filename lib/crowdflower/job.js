...
var crowdflowerURLEncode = require('../crowdfloweURLEncode');
/**
 * @constructor
 ...
 */
Job.prototype.create = function(jsonObject, callback){
    var keys = Object.keys(jsonObject);
    var path = '/v1/jobs?';
    var body = crowdflowerURLEncode(jsonObject);
 ...
 * @param callback {function} - A callback function.
 */
Job.prototype.update = function(jobid, jsonObject, callback){
    var keys = Object.keys(jsonObject);
 ...
 * @param callback {function} - A callback function.
 */
Job.prototype.delete = function(jobid, callback){
    var jobID = encodeURI(jobid);
...
Job.prototype.copyjob = function(jobid, allUnit, gold, callback) {
    ...
    if(arguments.length === 2){
        callback = allUnit;
        path = '/v1/jobs/' + jobID + '/copy?&key=' + this.apiKey;
    }
    else{
        allUnit = allUnit.toString();
        if(allUnit === 'true'){
            path = '/v1/jobs/' + jobID + '/copy?all_units=' + allUnit + '&key=' + this.apiKey;
        }
        else{
            path = '/v1/jobs/' + jobID + '/copy?gold=' + gold + '&key=' + this.apiKey;
        }
    }
...
Job.prototype.uploadfile = function(jobid, force, fileContent, content_type, callback){
    ...
    if(arguments.length === 4){
        ...
        if(force === 'true'){
            ...
            path = '/v1/jobs/upload?key=' + this.apiKey;
        }
    }
    else{
        var jobID = encodeURI(jobid);
        if(force === 'true'){
            ...
            path = '/v1/jobs/' + jobID + '/upload?' + 'key=' + this.apiKey;
        }
    }
